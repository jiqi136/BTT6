...
class 类一一百度号验证手机号(类一一公共库):  # 调用 类的模具 self.模具一一数据库()
    ...
    def 模具一一追加保存手机号码到文本(self):
        手机号码=self.手机号码+'\n'
        文本 = open("F:\影视发帖推广\可以注册手机号码.txt", 'a', encoding='UTF-8')
    ...
    def 模具一一判断手机号码已注册(self):
        ...
        for i in '12345':
            def 判断手机号码已注册提醒窗口():
                ...
                try:  #调用异常处理,应对易发生错误的位置
                    ...
                    if 手机号码已注册True == True:
                        ...
                        self.浏览器操作.find_element_by_xpath('//*[@id="TANGRAM__PSP_22__confirm_cancel"]').click()  # 点击 .click() 密码框
            ...
            time.sleep(1)  # 等待  # 增加延迟
            self.浏览器操作.find_element_by_xpath('//*[@id="TANGRAM__PSP_3__password"]').click()  # 点击 .click() 密码框
