...
class 类一一维护代理IP(类一一公共库):  # 调用 类的模具 self.模具一一数据库()
    def __init__(self):
        ...
        self.返回网页一链接组列表=self.模具一一gr无序网址列表请求返回网页内容(self.提取ip网址列表)# greq,并发数=20uests.imap(任务列
    ...
    def 模具一一gr无序网址列表请求返回网页内容(self, 网址列表, 分流数=100,并发数=20):  # grequests.imap(任务列
    ...
    def 模具一一检测代理gr无序网址列表请求返回网页内容(self,分流数=100, 并发数=20):  # grequests.imap(任务列

        初始网址列表数 = len(self.待检测代理类型IP端口列表)
        ...
        while 条件循环 == 1:
            ...
            for 代理IP in self.待检测代理类型IP端口列表[0:分流数]:
                if str(代理IP) in self.异常代理IP:
                    continue  # 跳过当前循环,继续进行下一轮循环

                IP端口分列表.append(代理IP)

            if len(self.待检测代理类型IP端口列表) > 分流数 - 1:  # break # 结束循环 continue # 跳过当前循环,继续进行下一轮循环
                self.待检测代理类型IP端口列表 = self.待检测代理类型IP端口列表[分流数:]
            else:  # 否则
                self.待检测代理类型IP端口列表 = []
            ...
            for 代理IP in IP端口分列表:
                ...
                任务 = grequests.get(百度网址, headers=self.头部信息, proxies=proxies)  # timeout=len(任务列表)//2,
                任务列表.append(任务)
            ...
            try:  # 调用异常处理,应对易发生错误的位置
                ...
                for 返回网页内容 in 返回网页内容集:
                    ...
                    if '<Response [200]>' in 网页内容文本 and "访问过于频繁" not in 返回网页内容.text:
                        ...
                        pass
                        # self.待检测代理类型IP端口列表.append(返回网页内容.url)

                此时数 = int(time.time())
                if len(self.待检测代理类型IP端口列表) == 0:
                    print('网页采集,全部完成')
