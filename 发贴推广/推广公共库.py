...
class 类一一公共库:  # 调用 类的模具 self.模具一一数据库()
    ...
    def 模具一一清除浏览器历史缓存(self):
        ...
        pag.PAUSE = 3  # 增加延迟 等待清除新窗口
        pag.moveTo(580, 690)  # 鼠标移动X.Y 方位  清除浏览历史书签 方框定位
        pag.rightClick()  # 右击
        ...
        pag.press('tab')  # press()一次完整的击键.hotkey('ctrl','c')
        time.sleep(0.3)  # 等待  # 增加延迟
        pag.press('enter')  # press()一次完整的击键.hotkey('c  trl','c')  确认 清除浏览历史书签  按钮
    ...
    def 模具一一重新激活浏览器窗口(self):
        ...
        time.sleep(0.5)  # 等待  # 增加延迟
        pag.moveTo(280, 920)  # 鼠标移动X.Y 方位  重新激活浏览器窗口 在CMD 与  回归桌面之间
        pag.rightClick()  # 右击
