...
def 条件控制():
    ...
    def 异常处理():
        ...
        assert(断言)#assert n != 0, 'n is zero!' .  assert的意思是，表达式n != 0应该是True，否则，根据程序运行的逻辑，后面的代码肯定会出错。
...
def MySQL数据库():
    ...
    def 对结果集进行排序():#ORDER BY 关键字默认按照升序对记录进行排序。如果需要按照降序对记录进行排序，您可以使用 DESC 关键字。
        列排序="""SELECT * FROM 中文表
            ORDER BY A列;"""
    ...
    def 修改表中已存在的数据(): # 更新
        ...
        def 批量修改多个字段():
            ...
            self.sql句2字段 =self.sql句2字段+ "WHEN '{}' THEN '{}' ".format(self.网址,文本否)
...
def 文本清洗():
    ...
    def 正则表达式与截去():
        ...
        文本 =re.sub(规则, 替换的文本, str(原文本),count=0,re.S|re.I)
        #替换文本为‘’时，就当作是截去
