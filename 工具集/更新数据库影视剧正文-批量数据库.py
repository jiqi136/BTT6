...
class 类一一更新正文(类一一公共库):  # 调用 类的模具 self.模具一一数据库()
    ...
    def 模具一一批量清洗重修正文(self):
        ...
        for 影视类型 in 影视类型列表:
            ...
            源码链接组分列表=[]
            总列表倒数 =len(self.源码链接组列表)
            for 源码链接组 in self.源码链接组列表:
                源码链接组分列表.append(源码链接组)
                总列表倒数 = 总列表倒数 - 1

            


                if len(源码链接组分列表) ==3000 or 总列表倒数 == 0:
                    倒数=len(源码链接组分列表)
                    ...
                    for 源码链接组 in 源码链接组分列表:
                        ...
                        self.模具一一帖子正文列表清洗二()
                        倒数=倒数-1
                        print(总列表倒数,'======清洗帖子正文倒数=======', 倒数)


                        if self.帖子内容 == self.正文:
                            print('======正文相同,跳过当前循环========')
                        ...
                        self.正文不相同 =200
                        sql句1字段 = sql句1字段 + "WHEN '{}' THEN '{}' ".format(self.网址, self.正文)
                        # sql句1字段 = sql句1字段 + "WHEN '{}' THEN '正文' ".format(self.网址)

                        sql句后条件 = sql句后条件 + "'{}',".format(self.网址)

                    源码链接组分列表 = []
                    if self.正文不相同 ==0:
                        print('======正文相同,跳过当前  更新数据库  循环========')
                        continue  # 跳过当前循环,继续进行下一轮循环

                    sql语句1字段首 = 'UPDATE `网站文章内容` SET `正文`= CASE `帖子链接`{}END '.format(sql句1字段)

                    sql句后条件 = str(sql句后条件).strip(',')  # 去除最后一个 ，号  默认则是去除空格
                    sql句后 = 'WHERE `帖子链接` IN ({})'.format(sql句后条件)

                    self.内容正文sql语句合成 = sql语句1字段首 + sql句后
    ...
    def 模具一一读取源码内容数据库(self):
        ...
        cursor = db.cursor()
        # SQL 查询语句   已下载内容网址
        sql = "SELECT `源码`,`帖子链接`  FROM `目标网站源码` WHERE 影视类型='{}'".format(self.影视类型)

        # 执行SQL语句
        cursor.execute(sql)
        # 获取所有记录列表
        self.源码链接组列表 = cursor.fetchall()
        # 关闭数据库连接
        db.close()

    def 模具一一读取内容数据库正文(self):
        # 连接数据库
        # 打开数据库连接
        ...
        cursor = db.cursor()
        # SQL 查询语句   已下载内容网址
        sql = "SELECT `正文`,`帖子链接`  FROM `网站文章内容` WHERE 影视类型='{}'".format(self.影视类型)

        # 执行SQL语句
        cursor.execute(sql)
        # 获取所有记录列表
        self.源码链接组列表 = cursor.fetchall()  #  正文 暂代为源码链接组
        # 关闭数据库连接
        db.close()
    ...
    def 模具一一读取网站文章内容删除发布是数据库(self):
        ...
        cursor.execute(sql)
        # 获取所有记录列表
        self.网址正文组列表 = cursor.fetchall()
        # 关闭数据库连接
        db.close()

    def 模具一一帖子正文列表清洗二(self):
    
        # 规则 = '(?<=648542).{1,}(?=BAIDU_CLB_SLOT_ID)'
        self.一楼内容 = str(self.帖子内容)
    ...
    def 模具一一替换不存在帖子的正文(self):
        ...
        for 帖子链接组 in self.不存在帖子链接组列表:
            ...
            for 正文组 in self.正文组列表:
                ...
                self.模具一一更新数据库里不存在的内容正文()

    def 模具一一更新数据库里不存在的内容正文(self):
        # 连接数据库
        print('连接数据库....')
        db = pymysql.connect("localhost", "root", "", self.新旧剧集, charset="utf8")
        # 获取游标
        ...
        sql = "UPDATE `网站文章内容` SET `正文`=('%s') WHERE `帖子链接`=('%s')" % (self.正文,self.网址)
    ...
    def 模具一一读取备份的内容数据库(self):
        ...
        cursor = db.cursor()
        # SQL 查询语句   已下载内容网址
        sql = "SELECT `正文` FROM `网站文章内容一一测试` WHERE 帖子链接='{}'".format(self.网址)
    ...
    def 模具一一读取内容数据库(self):
        ...
        cursor.execute(sql)
        # 获取所有记录列表
        self.网址组列表 = cursor.fetchall()
        # 关闭数据库连接
        db.close()

    def 模具一一更新数据库里不存在的发布(self):
        # 连接数据库
        print('连接数据库....')
        db = pymysql.connect("localhost", "root", "", self.新旧剧集, charset="utf8")
        # 获取游标
    ...
    def 模具一一更新数据库里内容正文(self):
        ...
        for i in 'ewhrhthtrhtrfcbevgretregre3':
            try:
                db = pymysql.connect("localhost", "root", "", self.新旧剧集, charset="utf8")
            except (RuntimeError) as 异常原因:  
                print('无法启动新线程...')
                 # 关闭数据库连接
                db.close()
                time.sleep(3)
        ...
        for i in 'ew3':
            try:
                ...
                # 提交到数据库执行
                db.commit()
            except (pymysql.err.ProgrammingError,pymysql.err.OperationalError) as 异常原因:
                # 如果发生错误则回滚
                db.rollback()
                print('更新出错', 异常原因)
                time.sleep(1)
        ...
        for 各帖子链接 in 帖子链接总列表:
            ...
            if len(帖子链接分列表) ==200 or 总列表倒数 == 0:
                ...
                for 内容页 in 网页内容列表:
                    ...
                    sql句2字段 =sql句2字段+ "WHEN '{}' THEN '{}' ".format(self.网址,文本否)
                    sql句后条件 = sql句后条件+"'{}',".format(self.网址)



                sql语句1字段首='UPDATE `网站文章内容` SET `正文`= CASE `帖子链接`{}END,'.format(sql句1字段)
                sql句2字段首='`发布` = CASE `帖子链接` {} END '.format(sql句2字段)
                ...
                # print('=====self.内容正文sql语句合成====', self.内容正文sql语句合成)
