...
def 模具一一文件翻译PY语言():
    ...
    while var == 1:  # 表达式永远为 true的无限循环 否则
        ...
        符号字典 = { '=':'=','。':'.','？':'?', '｛':'{','｝':'}',
             '……':'^','，':',','：':':','%':'%', '（':'(','）':')', '【':'[','】':']',
              '“': '\"','‘': '\'','’': '\'','编码U': 'encoding=\'UTF-8\'','编码G': 'encoding=\'gbk\''}#'   ,'时间':'time'
        py语法字典= {'输入':'等待用户输入=input("\=n按下 enter 确认键后继续")','时间':'time','返回':'return#返回','异常':'except #异常','断言':'assert #断言','结束':'assert #结束','否则':'else:  #否则',
             '文本前后':'strip(\'\')','以':'with','文本列表':'文本列表 = 文本.split("n")','列表最大值':'max(列表) #列表最大值 ',
              '范围':'range(1,11):#范围 1至10','从':'from','等待':'time.sleep(1) # 等待',
             '不换行':'end','自用':'if __name__ == \'__main__\':',
              '例':'self','浮点数':'float','打印':'print(\'：\',)','类型': 'print(\'类型:\',type(aaa))',
                '文本': 'str()', '整数': 'int()', '点数': 'float()', '集合': 'set()', '转换列表': 'list()', '输出': 'self.模具一一查看变量输出文本值("包管理", 包管理,8)',
