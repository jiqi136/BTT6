...
class 类一一公共库:  # 调用 类的模具 self.模具一一数据库()
    ...
    async def 模具一一内容页一分支异步打开网页(self):  # 定义(def)一个分支异步模具(async)
        ...
        while 条件循环 == 1:  # 条件循环  break # 结束循环 continue # 跳过当前循环,继续进行下一轮循环
            ...
            try:
                async with self.会话.get(self.各帖子链接, headers=头部信息, timeout=1) as 网页内容:
                ...
                次数循环 += 1
                print('网络异常等待', 异常)
                print('异步打开网页--倒数9.2秒再连接', 次数循环, '次','倒数',self.倒数)
                # time.sleep(3)
    ...
    def 模具一一内容页一分支异步调度(self):
        ...
        self.倒数 = len(self.页面提取帖子链接列表)  # [范围开始:范围结束]
    ...
    def 模具一一种子下载一分支异步调度(self):
        ...
        self.倒数 = len(self.目录与种子名一下载地址组列表)  # [范围开始:范围结束]

        for self.目录与种子名一下载地址组 in self.目录与种子名一下载地址组列表:
            # ==========任务调度=============
...
class 类一一电影下载(类一一公共库):  # 调用 类的模具 self.模具一一数据库()
    ...
    def 模具一一总提取帖子链接列表(self):
        ...
        if self.倒页总数 ==0:
            ...
            页数倒数=len(self.页数链接列表网页内容列表)
            for 页数链接列表网页内容 in self.页数链接列表网页内容列表:
                # ====================================================

                帖子列表html = etree.HTML(页数链接列表网页内容)
    ...
    async def 模具一一电影页数提取网址一分支异步打开网页(self):  # 定义(def)一个分支异步模具(async)
        ...
        while 条件循环 == 1:  # 条件循环  break # 结束循环 continue # 跳过当前循环,继续进行下一轮循环
            ...
            try:
                async with ahttp.get(url=self.各帖子链接) as 网页内容:

                    # 等待返回网页内容后,再return 返回模具的数据 , headers=头部信息,timeout=1
    ...
    def 模具一一电影页数提取网址一分支异步调度(self):
        ...
        #conn = aiohttp.TCPConnector(limit=5)  # 默认100，0表示无限  全网连接数
        conn = aiohttp.TCPConnector(limit_per_host=6)#一个网站端点最大连接数
        cookies = {'cookies_are': 'working'}

        self.会话 = aiohttp.ClientSession(cookies=cookies,connector=conn)
