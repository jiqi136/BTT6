...
class 类_公共模具: #调用 类的模具 self.模具_数据库()
    def __init__(self):
       pass
        ...
        while 循环 == 0:  # 条件循环
            ...
            try:
                ...
                time.sleep(1)
                    ...
                    requests.exceptions.ChunkedEncodingError) as 异常:
...
class 类_最新影视剧(类_公共模具):  # 调用 类的模具 self.模具_数据库()
    def __init__(self):
        global 换IP时间计数 # 时间计数全局变量声明
    ...
    def 模具_每日更新页面进入每个帖子(self):
        ...
        self.进入每个帖子列表 =set(进入每个帖子列表)# set#转换为集合,去重复
        print('帖子列表数:',len(self.进入每个帖子列表))

        # = set(进入每个帖子列表)#转换为集合

    def 模具_过滤网址查找数据库(self):
        global 过滤帖子网址  # 过滤帖子网址全局变量声明
    ...
    def 模具_已下载内容页网址(self):
        global 已下载内容页网址  # 已下载内容页网址全局变量声明
        db = pymysql.connect("localhost", "root", "", "最新影视剧", charset="utf8")
        # 使用cursor()方法获取操作游标
        cursor = db.cursor()
        # SQL 插入语句
        ...
        cursor.execute(sql)
        # 获取所有记录列表
        已下载内容页网址 = cursor.fetchall()
        # 已下载内容页网址 = str(已下载内容页网址)
               # 关闭数据库连接
        db.close()

    def 模具_过滤种子网址查找数据库(self):
        global 过滤种子网址  # 过滤种子网址全局变量声明
        db = pymysql.connect("localhost", "root", "", "最新影视剧", charset="utf8")
        # 使用cursor()方法获取操作游标
        cursor = db.cursor()
        # SQL 插入语句
        ...
        过滤种子网址2 = cursor.fetchall()
        过滤种子网址 = str(过滤种子网址2)

        # 关闭数据库连接
        db.close()
class 类_帖子内页内容(类_公共模具):  # 调用 类的模具 self.模具_数据库()
    def __init__(self,每个帖子网址):
        self.每个帖子网址=每个帖子网址
        self.模具_每个帖子提取内容()
    ...
    def 模具_提取标题图与过滤(self):
        ...
        self.标题图 = 标题图集[0]
        if 'http' not in self.标题图:
            self.标题图 = self.标题图.replace("/upload", "http://91btbtt.com/upload")
    ...
    def 模具_提取帖子标题(self):
        ...
        if '电影' in self.影视类型:  # 其它条件.: #  break # 结束循环 continue # 跳过当前循环,继续进行下一轮循环
            pass
        ...
        assert len(self.帖子标题) != 0, '帖子标题为空:'

    def 模具_长中短标题的再次清洗(self):
        短标题 = str(self.帖子标题)
        短标题.strip(' ')

        assert len(短标题) != 0, '短标题为空:'
        ...
        if ' ' in str(短标题):
            短标题 = str(短标题).replace(' ', "%" , 1)  # 替换   , 1) 次数 1
            规则 = '%.{1,}'
        ...
        if '第' in str(self.中短标题):
            if '季' in str(self.中短标题):
                ...
                季度名列表 = re.findall(季度名规则, str(self.中短标题))  # 提取列表
                季度名 = 季度名列表[0]
                季度名 = self.模具_符号清洗(季度名)
                self.短标题 = self.短标题+季度名
        print('短标题', self.短标题)

        assert len(self.短标题) != 0 ,'短标题为空:'
    ...
    def 模具_提取影视类型(self):
        ...
        if '剧集' in self.影视类型:  # break # 结束循环 continue # 跳过当前循环,继续进行下一轮循环
            ...
            网站副栏目ID10 = [网站副栏目字典[x] if x in 网站副栏目字典 else x for x in self.标题前的栏目列表]
            网站副栏目ID = str(网站副栏目ID10)
            星期数字典 = {'0': '141','1': '148', '2': '149', '3': '150', '4': '151', '5': '152', '6': '153', '7': '154'}
    ...
    def 模具_提取帖子正文与清洗(self):
        ...
        # ========字符替换=========
        列表 = ['<script type="text/javascript" ','<script type=\"text/javascript\" ',
              'src="http://cbjs.baidu.com/js/o.js"></div>','src="http://cbjs.baidu.com/js/o.js">',
              "迅雷", "百度网盘", "网盘下载", "http://pan.baidu.com", "https://pan.baidu.com", "ed2k://", "https://yunpan.cn",
    ...
    def 模具_提取每层种子(self):
        ...
        for 各层内容 in 各层内容列表:  # 范围
            ...
            if '个附件' not in 各层内容文本:  # break # 结束循环 continue # 跳过当前循环,继续进行下一轮循环
            ...
            if '动漫' in self.影视类型: #  break # 结束循环 continue # 跳过当前循环,继续进行下一轮循环
                if '猪猪' in 各层内容文本:
                    continue  # 跳过循环
            ...
            try:
                self.各层名 = 各层名[0]#调用异常处理,应对易发生错误的位置
            ...
            if '#1楼' in 各层内容文本:  # 其它条件.
                self.种子目录 = 'F:/电影模板/下载种子目录2/{}/{}/'.format(self.影视类型,self.短标题)  # 不换行 end=""

            else:
                self.种子目录 = 'F:/电影模板/下载种子目录2/{}/{}/{}/'.format(self.影视类型,self.短标题, self.各层名)
            for 各链接 in 各层链接列表:  # 范围 range
                ...
                下载地址 = str(各链接).replace("attach", "http://91btbtt.com/attach")
                self.下载地址 = str(下载地址).replace("dialog", "download")

                if self.下载地址 in 过滤种子网址:  # break # 结束循环 continue # 跳过当前循环,继续进行下一轮循环
                    print('跳过已过滤的下载种子')
                    continue  # 跳过当前循环,继续进行下一轮循环

                self.模具_建立目录并下载种子()

    def 模具_建立目录并下载种子(self):
        # global 过滤种子网址  # 过滤种子网址全局变量声明
        ...
        目录与种子名 = str(self.种子目录) + str(self.种子名)

        if os.path.exists(目录与种子名):  # 避免重复
            print('跳过已存在种子')
            return  # 返回

        self.模具_打开的网址请求返回网页内容(self.下载地址)

        try:
            with open(目录与种子名, 'wb') as fout:  # async

                fout.write(self.返回网页内容.content)  # await   .content
                fout.close()
    ...
    def 模具_插入数据库(self):
        ...
        cursor = db.cursor()
        # SQL 插入语句
        sql ="""INSERT INTO 剧集(影视类型,时间ID,长标题,短标题,主栏目ID,副栏目ID,摘要,正文,标题图,主演之标题前栏目字符串,文章来源之更新集数,帖子链接)
                                       VALUES ('%s','%d','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')""" \
                                  % (self.影视类型, self.时间ID, self.长标题, self.短标题, self.主栏目ID, self.网站副栏目ID, self.摘要, self.正文,
                                  self.标题图, self.标题前栏目字符串, self.更新集数, self.每个帖子网址) # %(self.影视类型)
        try:
            # 执行sql语句
            cursor.execute(sql)
            # 提交到数据库执行
        ...
        for 已下载种子网址 in self.已下载种子网址列表:  # 范围 range
            try:  # 已下载种子网址列表
                ...
                cursor.execute("""INSERT INTO 过滤种子网址(过滤种子网址)
                               VALUES ('%s')""" % (已下载种子网址))  #
    ...
    def 模具_每个帖子提取内容(self):

        self.循环结束 = 1
        self.已下载种子网址列表 = []
        if self.每个帖子网址 in 过滤帖子网址:
            print('过滤帖子网址')
            return  # 返回 跳过循环
        ...
        if self.每个帖子网址 in str(已下载内容页网址):
            print('已经存在于数据库')
