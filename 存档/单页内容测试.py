...
class 类_公共模具:  # 调用 类的模具 self.模具_数据库()
    ...
    def 模具_打开的网址请求返回网页内容(self, 网址):
        ...
        while 循环 == 0:  # 条件循环
            ...
            try:
                self.返回网页内容 = requests.post(网址, headers=self.头部信息, timeout=8)
...
class 类_最新影视剧(类_公共模具):  # 调用 类的模具 self.模具_数据库()
    def __init__(self):
        ...
        运行类 = 类_帖子内页内容(self.已下载内容页网址, self.过滤种子网址, self.过滤帖子网址)
        每个帖子网址='http://btbtt.co/thread-index-fid-950-tid-4272030.htm'
        运行类.模具_每个帖子提取内容(每个帖子网址)
    ...
    def 模具_进入置顶类型帖子(self):
        ...
        for 帖子位置数 in range(2 ,6):
            ...
            self.置顶类型帖子网址 = 'http://www.btbtt.net/' + str(置顶类型帖子[0])
...
class 类_帖子内页内容(类_公共模具):  # 调用 类的模具 self.模具_数据库()
    ...
    def 模具_提取标题图与过滤(self):
        ...
        if 'http' not in self.标题图:
            self.标题图 = self.标题图.replace("/upload", "http://www.btbtt.co/upload")
