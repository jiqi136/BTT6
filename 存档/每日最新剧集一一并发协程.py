...
class 类一一公共库:  # 调用 类的模具 self.模具一一数据库()
    ...
    def 模具一一提取最新影视剧数据库里的种子链接数(self):
        global 弹出新剧集提示计数  # def 函数模具内通行变量
        # 提取数据库里的过滤网址
        # 打开数据库连接
        db = pymysql.connect("localhost", "root", "", "最新影视剧", charset="utf8")
        # 使用cursor()方法获取操作游标
    ...
    def 模具一一提炼类型帖子最新各剧集链接(self):
        ...
        self.类型帖子最新各剧集链接 = set(类型帖子最新各剧集链接)  # 转换为集,去重复list()
    ...
    def 模具一一发贴时间半年前过滤(self):
        ...
        if 发帖时间天数 < 半年前时间 and '4378529' not in str(self.各帖子链接):  # break # 结束循环 continue # 跳过当前循环,继续进行下一轮循环
            self.跳过循环 = 0
            print('发贴时间为半年前,跳过循环:')
            return  # 返回
    ...
    def 模具一一提取长标题(self):
        def 模具一每框内容清洗(每框内容):
            清洗字列表 = ['BT', '网盘', '下载', '迅雷', '种子', '百度', 'MKV', '更至', '全集', '中字', '英语', '国语',
                     '日语', '中英', '字幕', 'MP4', 'MKV', '0P', '多版', '双语', '无水印', '打包', '[全', 'HQC', '片源', 'BD', 'RMVB', '韩语', '无字',
                     'WEBRip','GB']  # ,'GB','','','','','','','','','','','','','',''
            for 清洗字 in 清洗字列表:
                if 清洗字 in 每框内容:
                    每框内容 = ''
        ...
        if '更' in 内容 and '集' in 内容:
            规则 = '更.{1,}集'
            长标题 = re.sub(规则, '', 内容)
            # 符号清洗
        self.符号清洗不过滤空格 = 1
        self.长标题 = self.模具一一符号清洗(内容)
        if len(self.长标题) == 0:
            self.长标题 = self.多名长标题
        ...
        for 符号 in 符号列表:
            if 符号 in self.多名长标题:
                if 符号==' ':
                    符号='\s'
                规则 = '{}.{1,}'.format()#不换行 end=""
                短标题 = re.sub(规则, '', self.多名长标题, count=0)
    ...
    def 模具一一短标题的再次清洗(self):
        ...
        if '][' in str(self.短标题):
            规则 = '(?<=\]\[).{1,}'
            短标题列表 = re.findall(规则, str(self.短标题))
...
class 类一一置顶类型帖子分类(类一一公共库):  # 调用 类的模具 self.模具一一数据库()
    def __init__(self):
        global 半年前时间, 弹出新剧集提示计数  # def 函数模具内通行变量
    ...
    def 模具一一置顶类型帖子分类(self):
        ...
        for 置顶类型帖子网页内容 in 置顶类型帖子网页内容列表:
            ...
            if '剧集' in 置顶帖子类型标题文本:  # break # 结束循环 continue # 跳过当前循环,继续进行下一轮循环


                帖子类 = 类一一电视剧下载(self.类型帖子最新各剧集链接, self.电视剧类型)

            elif '动漫' in 置顶帖子类型标题文本:  # 其它条件.
                帖子类 = 类一一动漫下载(self.类型帖子最新各剧集链接, self.电视剧类型)
            else:  # 否则
                continue  # 跳过当前循环,继续进行下一轮循环

class 类一一电视剧下载(类一一公共库):  # 调用 类的模具 self.模具一一数据库()
    def __init__(self, 类型帖子最新各剧集链接, 电视剧类型):
        self.类型帖子最新各剧集链接 = 类型帖子最新各剧集链接
        self.电视剧类型 = 电视剧类型
    ...
    def 模具一一电视剧各链接网址处理(self):
        ...
        print('========================等待==============================')
        print(self.电视剧类型,'=剧集数=',len(self.类型帖子最新各剧集链接))

        内容页列表 = self.模具一一grequests打开网址随机无序集(self.类型帖子最新各剧集链接)
    ...
    def 模具一一电影各链接网址处理(self):
        ...
        for 各帖子链接 in self.页面提取帖子链接列表:
            if len(各帖子链接) == 0:
                ...
                continue  # 跳过循环  # 返回 结束任务
            elif 各帖子链接 in self.过滤帖子网址:
                print('==过滤帖子网址=== # 跳过当前循环')
                continue  # 跳过循环  # 返回 结束任务
            # =========请求各帖子链接网页内容======================
            self.各帖子链接 = self.模具一一网站图片网址清洗转换(各帖子链接)
            页面提取帖子链接列表.append(self.各帖子链接)

        网页内容列表 = self.模具一一grequests打开网址随机无序集(页面提取帖子链接列表)
        ...
        for 网页内容 in 网页内容列表:
            ...
            self.帖子内容 = 网页内容.text
            self.帖子内容html = etree.HTML(网页内容.text)
            self.各帖子链接 = 网页内容.url
