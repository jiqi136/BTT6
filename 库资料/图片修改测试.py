...
def 区域剪切截图():
    ...
    图片地址 = r'C:\下载中转站\Python调用示例\baidu.png'
    图片缓存 = Image.open(图片地址)
    四角坐标= (220,200,310,235)#（左，上，右，下）
    剪切的图 = 图片缓存.crop(四角坐标)
    剪切的图.show()# 显示


    #img_c = 图片缓存.crop([图片缓存.size[0] / 4, 图片缓存.size[1] / 4, 图片缓存.size[0] * 3 / 4, 图片缓存.size[1] * 3 / 4])
    #plt.imshow(img_c)
    del 图片缓存
    剪切的图.save(图片地址)  # 另存为
    print('截图完成')
