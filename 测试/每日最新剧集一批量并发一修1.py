...
class 类一一综合影视类型(类一一公共库):  # 调用 类的模具 self.模具一一数据库()
    ...
    """======================="""
    async def 模具一一异步打开网页一各链接网址处理(self, url):
        ...
        async with aiohttp.ClientSession(headers=头部信息, connector=aiohttp.TCPConnector(limit=10)) as 会话:
            ...
            try:  # 调用异常处理,应对易发生错误的位置
                ...
                self.帖子内容 = 返回网页
                self.各帖子链接 = url
                self.帖子内容html = etree.HTML(返回网页)
                self.模具一一综合影视各链接网址处理()


    def 模具一一异步调度一各链接网址处理(self,帖子链接分列表):
        事件循环 = asyncio.get_event_loop()
        此时数 = int(time.time())
        if 此时数 > 换IP时间计数 + 60:
            self.模具一一换ip连接()
        ...
        for 各帖子链接 in 帖子链接分列表:
            ...
            任务 = asyncio.ensure_future(self.模具一一异步打开网页一各链接网址处理(各帖子链接))
            任务列表.append(任务)
    ...
    def 模具一一综合影视各链接网址总处理(self):


        print('========================等待==============================')
        print(self.电视剧类型, '=剧集数=', len(self.页面提取帖子链接列表))
        self.页面提取帖子链接列表 = list(self.页面提取帖子链接列表)  # 集转换 为列表
