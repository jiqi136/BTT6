...
class 类_注册微软hotmail邮箱:
    ...
    def 模具_添加城通网盘号数据库(self):
        ...
        # 获取游标
        sql = """INSERT INTO 城通网盘号(邮箱帐号,密码,用户名,注册日期)
                   VALUES ('%s','%s','%s','%s')""" % (self.邮箱帐号,self.密码,self.用户名,self.今天时间)
        # 执行SQL语句
        cursor.execute(sql)
        ...
        # 关闭数据库连接
        db.close()
    def 模具_读取邮箱帐号数据库(self):

        # 连接数据库
        # 打开数据库连接
        db = pymysql.connect("localhost", "root", "",'微软邮箱帐号', charset="utf8")
        # 使用cursor()方法获取操作游标
        cursor = db.cursor()
        # SQL 查询语句   已下载内容网址
        sql = "SELECT 邮箱帐号 FROM 剧集"
        # 执行SQL语句
        cursor.execute(sql)
        # 获取所有记录列表
    ...
    def 模具_浏览器输入验证码(self):
        ...
        # 更换头部
        option.add_argument("''User-Agent': 'Mozilla/5.0(WindowsNT6.2)AppleWebKit/536.3(KHTML,likeGecko)Chrome/19.0.1061.1Safari/536.3'")
        ...
        # ,
        driver.find_element_by_xpath('//*[@id="domainLabel"]').click()  # 定位确认 邮箱类型
        time.sleep(1)  # 等待
        driver.find_element_by_xpath('//*[@id="domain1"]').click()  # 定位确认 选择 hotmail
        self.密码="qq{}Q%".format(self.密码数字)
        print('密码..........',self.密码)



        driver.find_element_by_xpath('//*[@id="MemberName"]').send_keys(self.随机帐号)  # 定位用户名
        driver.find_element_by_xpath('//*[@id="Password"]').send_keys(self.密码)  # 定位密码

        driver.find_element_by_xpath('//*[@id="iOptinEmail"]').click()  # 定位确认不接收广告邮件
        driver.find_element_by_xpath('//*[@id="iSignupAction"]').click()  # 定位确认 下一步
        print('输入姓名......')
        time.sleep(2)  # 等待
        driver.implicitly_wait(20)  # 设置 隐式等待.单位是秒（s）

        driver.find_element_by_xpath('//*[@id="LastName"]').send_keys(self.百家姓)  # 定位 姓
        driver.find_element_by_xpath('//*[@id="FirstName"]').send_keys(self.百家名)  # 定位 姓
        driver.find_element_by_xpath('//*[@id="iSignupAction"]').click()  # 定位确认 下一步
        ...
        print('选择地区与日期')
        time.sleep(1)  # 等待
        driver.implicitly_wait(20)  # 设置  隐式等待（s）,


        driver.find_element_by_xpath('//*[@id="BirthYear"]').click()  # 定位 年
        time.sleep(1)  # 等待
        driver.find_element_by_xpath('//*[@id="BirthYear"]/option[{}]'.format(self.年)).click()  # 定位 年

        driver.find_element_by_xpath('//*[@id="BirthMonth"]').click()  # 定位月
        time.sleep(1)  # 等待
        driver.find_element_by_xpath('//*[@id="BirthMonth"]/option[{}]'.format(self.月)).click()  # 定位月

        driver.find_element_by_xpath('//*[@id="BirthDay"]').click()  # 定位日
        time.sleep(1)  # 等待
        driver.find_element_by_xpath('//*[@id="BirthDay"]/option[{}]'.format(self.日)).click()  # 定位日

        driver.find_element_by_xpath('//*[@id="iSignupAction"]').click()  # 定位下一步

        print('输入验证码.')
        # driver.find_element_by_xpath('//*[@id="wlspispSolutionElement787c8dfd33ff43f79f6b6673430da761"]').click()  # 定位验证码
        input("\n\n按下 确认键enter 键后继续.")

        driver.find_element_by_xpath('//*[@id="iSignupAction"]').click()  # 定位 验证码 的下一步
        print('保存帐号信息')
        ...
        driver.implicitly_wait(30)  # 设置  隐式等待（s）

        print('选择项卡')

        driver.find_element_by_xpath('/html/body/section/div/div[2]/section/button/i').click()  # 定位 滑动方向
        driver.implicitly_wait(2)  # 设置  隐式等待（s）
        driver.find_element_by_xpath('/html/body/section/div/div[2]/section/button[2]/i').click()  # 定位 滑动方向
        driver.implicitly_wait(2)  # 设置  隐式等待（s）
        driver.find_element_by_xpath('/html/body/section/div/div[2]/section/button[2]/i').click()  # 定位 滑动方向
        driver.implicitly_wait(2)  # 设置  隐式等待（s）
        driver.find_element_by_xpath('/html/body/section/div/div[2]/section/button[2]/i').click()  # 定位 滑动方向
        driver.implicitly_wait(60)  # 设置  隐式等待（s）


        print('全部就绪')
        driver.find_element_by_xpath('/html/body/section/div/div[2]/section/section/section/div/button').click()  # 定位 开始吧
        time.sleep(10)  # 等待
        print('登录成功，保存帐号信息')
